...
def changeRequrePath():
    try:
        ...
        gamePath = os.path.join(gamePath, copyDirName, "compiled.js")
        f = open(gamePath, "r")
        ...
        for line in data_list:
            ...
            if replaceList[3] in line:
                print("删除导出模块 cmsg ！！")
                line = line.replace(replaceList[3], 'const cmsg')
    ...
    try:
        ...
        tsdPath = os.path.join(tsdPath, copyDirName, "compiled.d.ts")
        tsdf = open(tsdPath, "r")
        data_list = tsdf.readlines()
        starLine = 0
        data_list.insert(starLine, "declare global {\n")
        data_list.append("}\n")
        data_list.append("export { cmsg as default };")
        tsdf = open(tsdPath, 'w')
        tsdf.writelines(data_list)
        tsdf.flush()
        print("------compiled.d.ts 更换完毕-----")
    finally:
        if tsdf:
            tsdf.close()
def worksFiles():
    ...
    for filename in file_list:
        ...
        if portion[1] == ".js" or portion[1] == ".ts":
            ...
            target_path = os.path.join(pardir, filename)
